function CHL = get_NPQcorr_currentBGCversion(gps, data, dirs)
% I grabbed this function off the BGC github on April 27, 2023 to compare
% with the current version I am using for the float fluoresence correction
% Git hub function location:
% https://github.com/SOCCOM-BGCArgo/ARGO_PROCESSING/blob/master/MFILES/FLOATS/get_NPQcorr.m
% ************************************************************************
% PURPOSE: 
...
tTMLD    = abs(data(:,iT) - mean(data(ref_ind,iT),'omitnan')) <= mld_temp_threshold; 
...
%BBP    = data(:,iB);
CHLA   = data(:,iC);
dp     = mean(abs(diff(P(P<100))),'omitnan');
...
Zmin   = min([CHL.Dmld, CHL.Zeu],[],'omitnan');
